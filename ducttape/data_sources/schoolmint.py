...
class SchoolMint(WebUIDataSource, LoggingMixin):
    ...
    def _extract_html_body(self, msg):
        ...
        Walks a Gmail API message payload to find and decode the text/html body.
        """
        payload = msg['payload']
    ...
    def _get_latest_schoolmint_code_from_mfa_email(self, service, max_results=5, max_age_minutes=5):
        """
        [Generated by Claude]

        Fetches the most recent SchoolMint verification code email via the Gmail API
        and extracts the 6-digit code from its HTML body.

        Args:
            service: an authenticated Gmail API service object
            max_results: how many recent matching messages to consider, in case the
                very latest isn't the verification email for some reason
            max_age_minutes: reject any matching email older than this many minutes,
                to avoid picking up a stale code from a previous login attempt

        Returns:
            The 6-digit code as a string.

        Raises:
            NoDataError: if no matching, sufficiently recent email with an
                extractable code is found.
        """
        CODE_SPAN_PATTERN = re.compile(
            r'<span[^>]*letter-spacing[^>]*>\s*(\d{6})\s*</span>'
        )
        GENERIC_CODE_PATTERN = re.compile(r'\b(\d{6})\b')

        SCHOOLMINT_SENDER = 'services.schoolmint.com'

        query = 'from:{} subject:"Verification Code"'.format(SCHOOLMINT_SENDER)
        ...
        if not messages:
            raise NoDataError('No SchoolMint verification code emails found.')
        ...
        for msg_meta in messages:
            ...
            match = CODE_SPAN_PATTERN.search(html_body)
            if not match:
                # fall back in case SchoolMint tweaks their template
                match = GENERIC_CODE_PATTERN.search(html_body)

            if match:
                return match.group(1)

        if stale_count == len(messages):
            raise NoDataError(
                'Found SchoolMint email(s) but all were older than {} minutes.'.format(max_age_minutes)
            )
        raise NoDataError('Found SchoolMint email(s) but could not extract a code.')
    
    def _enter_mfa_code_into_prompt(self, mfa_code):
        """
        [Generated by Claude]

        Enters a 6-digit MFA code into SchoolMint's split-digit verification
        prompt, one digit per input box, and checks whether the code was
        accepted.

        Waits up to 10 seconds for the MFA input row to appear; if it never
        appears, assumes no MFA challenge was presented and returns silently
        (no-op). If the prompt does appear, fills each of the 6 `.mfa-code-cell`
        inputs with the corresponding digit from `mfa_code`, then waits briefly
        to see whether SchoolMint displays a rejection message. If a rejection
        is shown, quits the driver and raises InvalidLoginCredentials.

        Args:
            mfa_code (str): A 6-digit numeric string (e.g. '745154') to enter
                into the prompt.

        Raises:
            ValueError: If `mfa_code` is not exactly 6 digits.
            NoSuchElementException: If the MFA row is present but doesn't
                contain exactly 6 digit input cells.
            InvalidLoginCredentials: If SchoolMint reports the code as
                incorrect or expired.

        Note:
            No exception is raised if the MFA prompt never appears at all —
