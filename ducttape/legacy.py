...
def login_to_illuminate_selenium(driver, host, username, password):
    ...
    time.sleep(3)
    elem = driver.find_element(By.ID, "button_login") # actuate the 'login' key (we can just log in using the default site)
    elem.click()
    time.sleep(3)
...
def login_to_chalk_selenium(driver, host, username, password):
    ...
    elem = driver.find_element(By.ID, "session_email")
    elem.clear()
    elem.send_keys(username)
    elem = driver.find_element(By.ID, "password")
    elem.send_keys(password)
